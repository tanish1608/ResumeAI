...
export const exportToPDF = async (
  ...
  try {
    ...
    const bottomMargin = 5; // Smaller bottom margin for more content space
    ...
    [personalInfo.linkedin, personalInfo.github, personalInfo.website].forEach(
      (link) => {
        if (!link) return;
    ...
    if (
      ...
      checkPageBreak(20);
      addText("PROFESSIONAL SUMMARY", margin, currentY, 12, "bold");
      currentY += 2;
      addLine(currentY);
      ...
      const summaryLines = wrapText(
        resumeData.professionalSummary,
        contentWidth + 4, // Slightly reduced to ensure proper wrapping
        11
      );
    ...
    if (resumeData.education.length > 0) {
      ...
      resumeData.education.forEach((edu) => {
        ...
        addText(
          gpa || "",
          margin + schoolWidth + degreeWidth,
          currentY,
          11,
          "italic"
        );
        ...
        if (edu.relevantCoursework) {
          ...
          const courseworkLines = wrapText(
    ...
    if (resumeData.skills.length > 0) {
      ...
      resumeData.skills.forEach((skill) => {
        ...
        // Remaining lines: just skills with proper indentation at left margin
        for (let i = 1; i < skillsLines.length; i++) {
          checkPageBreak(6);
          addText(skillsLines[i], margin, currentY, 11, "normal");
          currentY += lineHeight;
        }
    ...
    if (resumeData.experience.length > 0) {
      ...
      resumeData.experience.forEach((exp) => {
        ...
        addText(
          companyLocationText,
          margin + titleWidth + companyNameWidth,
          currentY,
          11,
          "italic"
        );
    ...
    if (resumeData.projects.length > 0) {
      ...
      resumeData.projects.forEach((proj) => {
        ...
        if (proj.link) {
          ...
          const linkUrl = proj.link.startsWith("http")
            ? proj.link
            : `https://${proj.link}`;
          addText(titleText, margin, currentY, 11, "bold");
          // Make title clickable
          const titleWidth = pdf.getTextWidth(titleText);
          pdf.link(margin, currentY - 3, titleWidth, lineHeight, {
            url: linkUrl,
          });
        } else {
          addText(titleText, margin, currentY, 11, "bold");
